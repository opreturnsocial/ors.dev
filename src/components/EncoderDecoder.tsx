...
const V0_TXID =
  "aea0bc2b75151bfcb564279980ae222e76d8a8614c05e9513a9040bacfbbf536";

// First mined ORS v1 post chunk txids
const CHUNK0_TXID =
  "f31d17c819eeccf43f9a3a78101bc835e8d9158e3a7476f979effa19e92f6012";
const CHUNK1_TXID =
  "bbb060c131fcfcba41123465b3ccfd97dfa6889dcd5870e8d7748a8c2c57eaea";
...
function HexOutput({ label, hex }: { label: string; hex: string }) {
  return (
    <div className="space-y-1">
      ...
      <div className="font-mono text-xs bg-muted rounded p-3 break-all">
        {hex}
      </div>
    </div>
  );
  ...
  const verify = useCallback(async () => {
    ...
    try {
      if (post.version === "v0") {
        ...
        setSigValid(
          verifySignature(post.pubkey, post.sig, sha256(signingBody)),
        );
      }
    } catch {
      setSigValid(false);
    ...
    <div className="space-y-4 p-4 rounded-lg border bg-card">
      ...
      <div className="grid gap-3">
        ...
        <div>
          ...
          <div className="rounded p-3 border bg-background text-sm whitespace-pre-wrap">
            {post.content}
          </div>
        </div>
      </div>
...
function DecodeTab() {
  ...
  const decode = useCallback(() => {
    ...
    if (ver === "v0") {
      ...
      if (!chunk0) {
        setError("Failed to parse v1 chunk 0");
        return;
      }

      if (chunk0.totalChunks && chunk0.totalChunks > chunks.length) {
        setError(
          `This post needs ${chunk0.totalChunks} chunks. Add ${chunk0.totalChunks - chunks.length} more chunk input(s) below.`,
        );
        return;
      }

      const parsed = chunks.map((c, i) =>
        i === 0 ? chunk0 : parseV1Chunk(c.trim()),
      );
      const failed = parsed.findIndex((p) => !p);
      if (failed !== -1) {
        setError(`Failed to parse chunk ${failed}`);
        return;
      }
  ...
  const loadFirstPost = useCallback(async () => {
    ...
    try {
      ...
      if (!chunk0 || !chunk1) {
        setError("Failed to parse fetched chunks");
        return;
      }
    ...
    try {
      ...
      if (keyMethod === "generate") {
        ...
        if (!w.nostr.signSchnorr) {
          setError(
            "Your extension does not support signSchnorr - try Alby Extension",
          );
          setSigning(false);
          return;
        }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs ${contentBytes > 277 ? "text-red-500" : "text-muted-foreground"}`}
          >
            {contentBytes} bytes
          </span>
        </div>
      ...
      <div className="space-y-3 p-4 rounded-lg border bg-muted/30">
          ...
          <div className="space-y-2">
            ...
            {generatedPub && (
              <div className="space-y-1">
                ...
                <div className="font-mono text-xs bg-background rounded p-2 break-all">
                  {generatedPub}
                </div>
                <div className="text-xs text-muted-foreground">
                  Private key (keep secret)
                </div>
                <div className="font-mono text-xs bg-background rounded p-2 break-all text-muted-foreground">
                  {generatedPriv}
                </div>
              </div>
            )}
        ...
        {keyMethod === "extension" && (
          <p className="text-xs text-muted-foreground">
            Uses <code>window.nostr.signSchnorr()</code> from the Alby browser
            extension to sign the message hash. The private key never leaves
            your extension.
          </p>
        )}
      </div>
          ...
          {output.chunks ? (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Post split into {output.chunks.length} chunks. Broadcast each
                transaction separately and ensure both are confirmed within 6
                blocks.
              </p>
              {output.chunks.map((chunkHex, i) => (
                <div key={i} className="space-y-2">
                  <HexOutput
                    label={`Chunk ${i} - OP_RETURN data`}
                    hex={chunkHex}
                  />
                </div>
              ))}
